...
- [ ] Шаг 1.1: Открыть `public/resumeData.json` и полностью заменить секции `main`, `education`, `work` и `portfolio.projects` на подготовленный контент с фамилией Smazal und новыми ссылками.
- [ ] Шаг 1.2: Проверить, что даты обоих бакалавриатов (2010 и 2021) и периоды работы в РФ с точными месяцами прописаны без синтаксических ошибок в JSON.
...
- [ ] Шаг 3.2: Переписать рендер компонента с использованием хука `useState` для фильтрации проектов. Проекты должны фильтроваться по массиву `project.tags`.
- [ ] Шаг 3.3: Добавить стили кнопок фильтров в `src/App.css` (класс `.filter-btn` с изменением цвета при `.active` на фирменный `#11abb0`).
