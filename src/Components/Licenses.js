...
class Licenses extends Component {
  render() {
    ...
    if(this.props.data){
      var certificates = this.props.data.certificates.map(function(certificates, index){
        var certificateImage = process.env.PUBLIC_URL + '/images/licenses/'+certificates.image;
        return <div key={`${certificates.title}-${index}`} className="columns licenses-item">
